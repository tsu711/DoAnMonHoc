...
const FilterTop = () => {
  ...
  const [categories, setCategories] = useState({ name: "Tất cả sản phẩm" });
  ...
  useEffect(() => {
    ...
    const sortOption = location.pathname.split("/")[1];
    let sortQuery;
    if (sortOption === "listingSortAsc") {
      sortQuery = "price-asc";
    } else if (sortOption === "listingSortDesc") {
      sortQuery = "price-desc";
    }

    GET_PAGE(`products`, currentPage - 1, numItems, categoryId, sortQuery).then(
      (response) => {
  ...
  }, [categoryId, currentPage, location.pathname]);
