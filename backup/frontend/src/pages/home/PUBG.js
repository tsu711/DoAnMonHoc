...
const PUBG = () => {
	...
	useEffect(() => {
		...
		GET_ALL(`products/getlatest?categoryid=2`).then((item) => setProducts(item.data));
	  }, []);
	  useEffect(() => {
	...
	return (
		...
		<h3 className="title-section">
            {categories.length > 0 ? categories[1].categoryName : ""}
          </h3>
		</div>
		  ...
		  <div className="card card-home-category">
			<div className="row no-gutters">
			  <div className="col-md">
				...
				<Link to="/listing?categoryId=2" className="btn btn-primary">
                Xem thêm
              </Link>
			  </div>
			</div>
